...
public void renderButton(int x, int y, int mouseX, int mouseY) {
        ...
        GLRectUtils.drawRoundedRect(x, y, x + w, y + h,4.0f, new Color(0, 0, 0, animate.getValueI() + 100).getRGB());

        if(getText() == "Multiplayer") {
                GLRectUtils.drawRoundedOutline(x, y, x + w, y + h, 4.0f, 2.0f, new Color(60, 232, 118, animate.getValueI() + 100).getRGB());
        } else {
                GLRectUtils.drawRoundedOutline(x, y, x + w, y + h, 4.0f, 2.0f, new Color(60, 232, 118, animate.getValueI() + 100).getRGB());
        }
        ...
        text,
        x + w / 2f - FontUtil.regular.getStringWidth(text) / 2f,
        y + h / 2f - 3,
        -1
        );
        }
