...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn) {
        ...
        for (NetworkPlayerInfo networkplayerinfo : list) {
            ...
            // insert thing here lalalalala
            UserProfile userProfile = ChatClient.INSTANCE.getUserByMcName(networkplayerinfo.getGameProfile().getName());
            if (userProfile != null) {
                String text = userProfile.getRank() == UserProfile.Rank.ADMIN ? "§3" + userProfile.getUsername() : "§b" + userProfile.getUsername();
                k = this.mc.fontRendererObj.getStringWidth(this.getPlayerName(networkplayerinfo) + " §7(" + text + "§7) ");
            }
        ...
        for (int k4 = 0; k4 < l3; ++k4) {
            ...
            if (k4 < list.size()) {
                ...
                if (networkplayerinfo1.getGameType() == WorldSettings.GameType.SPECTATOR) {
                    ...
                    // insert thing here lalalalala
                    UserProfile userProfile = ChatClient.INSTANCE.getUserByMcName(gameprofile.getName());
                    if (userProfile == null) {
                        this.mc.fontRendererObj.drawStringWithShadow(s1, (float) j2, (float) k2, -1);
                    } else {
                        String text = userProfile.getRank() == UserProfile.Rank.ADMIN ? "§d" + userProfile.getUsername() : "§b" + userProfile.getUsername();
                        this.mc.fontRendererObj.drawStringWithShadow(s1 + " §7(" + text + "§7)", (float) j2, (float) k2, -1);
                    }
                }
